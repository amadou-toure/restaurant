...
isAuthenticated = (req, res, next) => {
  if (req.isAuthenticated() && (req.user.isCustomer() || req.user.isAdmin()))
    return next();
...
router.post(
  ...
  [
    check("firstname")
      .isLength({ min: 5 })
      .withMessage("firstname should be more than 5 char"),
    check("lastname")
      .isLength({ min: 5 })
      .withMessage("lastname should be more than 5 char"),
    check("contact")
      .isLength({ min: 3 })
      .withMessage("contact should be more than 5 char"),
    check("email")
      .isLength({ min: 5 })
      .withMessage("email should be more than 5 char"),
  ],
...
router.get(
  "/single/:id",
  isAuthenticated,
  customerController.find_customer_by_id
);
...
router.get(
  "/edit/:id",
  isAuthenticated,
  customerController.edit_customer_by_id
);
...
router.post(
  "/delete",
  isAuthenticated,
  customerController.delete_customer_by_id
);
