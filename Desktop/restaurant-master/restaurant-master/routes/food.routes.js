...
isAuthenticated = (req, res, next) => {
  if (req.isAuthenticated() && (req.user.isFooDrink() || req.user.isAdmin()))
    return next();
...
router.post(
  ...
  [
    ...
    check("category")
      .isLength({ min: 5 })
      .withMessage("category should be more than 5 char"),
    check("description")
      .isLength({ min: 5 })
...
router.post(
  ...
  [
    ...
    check("price")
      .isLength({ min: 5 })
      .withMessage("price should be more than 5 char"),
    check("type")
      .isLength({ min: 5 })
      .withMessage("type should be more than 5 char"),
    check("category")
