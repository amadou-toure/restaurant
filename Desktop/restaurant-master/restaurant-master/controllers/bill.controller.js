...
exports.new_bill = function (req, res) {
  ...
  if (!errors.isEmpty()) {
    ...
    // ajouter dannées dans la database
    newBill.save((err) => {
      if (!err) {
...
exports.all_bill = function (req, res) {
  ...
  let totalDocs = 0;
  Bill.countDocuments({}, (err, total) => {}).then((response) => {
